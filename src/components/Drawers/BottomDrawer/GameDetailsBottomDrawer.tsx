...
import { ListWithOwnership, UserLists } from '@/shared/types'
...
enum GameDetailsBottomDrawerDialogTypes {
	ADD,
	UPDATE
}

const GameDetailsBottomDrawer = ({ open, close }: DrawerProps) => {
	const [isDialogOpen, setIsDialogOpen] = React.useState<boolean>(false)
	const [_, setCurrentlySelectedList] = useCurrentlySelectedList()
	const [dialogType, setDialogType] = React.useState<GameDetailsBottomDrawerDialogTypes>()
	const [selectedGame] = useCurrentlySelectedGame()
	const lists = useListsWithOwnership()(selectedGame?.game_id)
	const handleOpenDialog = (listName: string, dialogType: GameDetailsBottomDrawerDialogTypes) => {
	...
	return (
		<BaseBottomDrawer open={open} close={close}>
								...
								list.listName !== UserLists.BACKLOG && (
									<div key={`${list}_${i}`}>
										<div
											onClick={() =>
												list.hasGame
													? handleOpenDialog(list.listName, GameDetailsBottomDrawerDialogTypes.UPDATE)
													: handleOpenDialog(list.listName, GameDetailsBottomDrawerDialogTypes.ADD)
											}
			...
			{dialogType === GameDetailsBottomDrawerDialogTypes.ADD ? (
